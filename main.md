...
This Request for Comment (RFC) is intended to propose a unified, interoperable standard specifically designed for the exchange of μseismic data generated by mine μseismic monitoring systems. Central to the proposed standard is the assurance of a lossless data export, combined with the comprehensive inclusion of all requisite data elements (waveform data, inventory information including the sensor location type and response file, and catalogue data), required for optimal utilization. It is pertinent to note that the primary architecture of this standard is not engineered for efficient channel-by-channel data recovery from disk storage. More specialized formats, such as TileDB, Zarr, or ASDF, would be recommended for such specific applications.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5MzYxNjExNzMsMTc0MjIzNiwtMTY2Mz
